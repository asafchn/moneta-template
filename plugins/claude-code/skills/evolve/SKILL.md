...
1. Read `.agent-evolve.md`. Invoke `evolve-init` if no connection exists. Resolve and retain the active domain/agent binding; ask when ambiguous. Verify and refresh the reviewed `base-branch` using [reviewed state](../knowledge-search/references/reviewed-state.md). Create the candidate branch in a separate local Git worktree/checkout from that reviewed revision, normally under `.evolve-local/candidates/`. Keep the configured retrieval checkout on reviewed state and preserve unrelated work.
...
3. Invoke `evolve-agent` to analyze that source in a dedicated native child. Pass the source path, coverage limits, active domain/agent, reviewed graph path/base revision, separate candidate checkout path, applicable role-file paths/revisions and expected outcome. The child follows [the analysis role](../evolve-agent/references/role.md). A child does not automatically inherit the whole source session.
