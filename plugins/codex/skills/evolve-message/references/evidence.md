...
Store a lightweight outcome note beside the assessment for this occurrence: source locator, candidate lesson/scope, no-change/deferred/review outcome and review URL if any. A full assessment uses the existing evaluation-result schema; no new operational node type is needed. A hook re-delivery reuses the occurrence's record and pending review. Uncertain identity requires checking the current context, not suppressing a new correction solely because its words repeat.
