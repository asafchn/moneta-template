...
function classify(stderr) {
  if (/Could not resolve to a Repository|HTTP\s*404\b|\b404 Not Found\b/i.test(stderr)) return 'not-found-or-inaccessible';
  if (/socket|connectex|ENETUNREACH|ECONNREFUSED|ENOTFOUND|EAI_AGAIN|could not resolve (?:host|hostname|proxy)|dial tcp|timed? out|TLS|certificate|proxy connect/i.test(stderr)) return 'network-blocked';
...
function main(args) {
  ...
  if (mode === 'host') {
    if (!['codex','claude'].includes(provider) || rest[0] !== 'plugin') return {status:'command-blocked',exitCode:null};
    const operation = rest[1] === 'marketplace' ? rest[2] : rest[1];
    if (!['add','install','enable','list','details','upgrade','update'].includes(operation) || rest.some(arg => redact(arg) !== arg)) return {status:'command-blocked',exitCode:null};
    if (process.platform === 'win32') {
      const quoted = rest.map(arg => "'" + arg.replace(/'/g,"''") + "'").join(' ');
      return runFiltered('powershell.exe',['-NoProfile','-NonInteractive','-Command',"& '"+provider+"' "+quoted+"; exit $LASTEXITCODE"],{timeout:120000});
    }
    return runFiltered(provider,rest,{timeout:120000});
  }
