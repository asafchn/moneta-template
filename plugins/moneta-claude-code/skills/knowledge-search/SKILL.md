...
1. Read `.moneta.md`; invoke `evolve-init` if absent. Resolve the active `(domain, agent)` binding from its role file or explicit input; ask if ambiguous. Apply [reviewed state](references/reviewed-state.md) before reading any graph file. Then verify the selected responsibility node through `indexes/agent-index.md`. Completion: one active identity, clean reviewed revision and domain path under `graph-root` are known.
