...
1. Invoke a separate `eval-agent` with [its role](../../evolve-evaluate/references/role.md) and `evolve-evaluate`. Pass the exact candidate diff/revision, base, active domain/agent, original request, evidence and `source-mode`. Completion: a retained assessment separates actual deterministic checks, task-fulfillment judgments and limits.
