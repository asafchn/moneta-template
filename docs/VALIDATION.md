...
Motion refinement in setup 0.10.4 / runtime 0.3.4: main node positions and selection ring are fixed; less prominent background nodes float and connection signals remain animated. The browser regression verifies all eight main positions stay identical while a background node moves, plus pause and reduced-motion playback. Five landing browser tests, two local-viewer browser tests, thirteen Python tests, both affected builds and package parity passed.
