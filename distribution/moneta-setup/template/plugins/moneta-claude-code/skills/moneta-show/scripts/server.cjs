...
// ../native/shared/scripts/redact.cjs
var require_redact = __commonJS({
  "../native/shared/scripts/redact.cjs"(exports2, module2) {
    "use strict";
    var MASK = "[REDACTED]";
    var credentialName = /token|secret|passw(?:or)?d|credential|key|authorization/i;
    var patterns = [
    ...
    module2.exports = { redact: redact2 };
  }
});
...
function readGraph(area) {
  ...
  const nodes = [], edges = [], warnings = /* @__PURE__ */ new Set(), byId = /* @__PURE__ */ new Map();
  for (const [type, definition] of Object.entries(types)) {
    ...
    for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
      ...
      if (nodes.length >= 2e3) throw new Error("The local viewer supports up to 2000 nodes per area. Narrow the area before viewing.");
  ...
  const result = clean({ scope: path.basename(root), nodes, edges, warnings: [...warnings], loadedAt: (/* @__PURE__ */ new Date()).toISOString() });
...
function createViewer(area, assets, beforeRead = () => ({ status: "current" })) {
  ...
  const server = http.createServer((request, response) => {
    ...
    if (request.headers.host !== origin.slice(7) || request.headers.origin && request.headers.origin !== origin || request.headers["sec-fetch-site"] === "cross-site") {
      ...
      response.end("Local viewer access only.");
      return;
    }
    ...
    if (url.pathname === "/graph.json") {
      ...
      try {
        ...
        response.end('{"error":"Unable to read selected graph. Check local schemas, node frontmatter, target paths and viewer limits."}');
      }
      return;
    }
    try {
...
Read only. Stop this process to close the viewer. Expires after 8 hours.`);
    if (!args.includes("--no-open")) {
      ...
      execFile(command, openArgs, { windowsHide: true }, (error) => {
        if (error) console.log("Open the localhost URL above in your browser.");
      });
    }
    setTimeout(() => {
  ...
  server.on("error", () => {
    console.error("Could not start the localhost viewer.");
    process.exitCode = 1;
  });
}
if (require.main === module) {
  try {
    ...
    console.error("Moneta viewer could not read the selected area. Check its catalogs, frontmatter, paths and node limits locally.");
    process.exitCode = 1;
  }
}
