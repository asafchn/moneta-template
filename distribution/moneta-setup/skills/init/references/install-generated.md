...
Create the private local connection/cache through [runtime connection](../../../template/plugins/moneta-codex/skills/knowledge-search/references/connection.md). Organization matching requires no per-codebase file. Register a Codex custom role only if explicitly requested: preserve conflicting project files; the runtime can otherwise delegate with its bundled role text. Claude discovers its bundled agents.
