...
function redact(input, env = process.env, {localPaths = []} = {}) {
  ...
  // Only explicitly resolved connection paths are exempt from the opaque-entropy heuristic.
  // Known secrets and credential patterns have already been removed above.
  const spans = [];
