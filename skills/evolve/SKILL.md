...
3. Invoke `evolve-agent` to analyze that source in a dedicated native child. Pass `source-mode: session`, the source path, coverage limits, active memory-scope/agent, reviewed graph path/base revision, separate candidate checkout path, applicable role-file paths/revisions and expected outcome. The child follows [the analysis role](../evolve-agent/references/role.md). A child does not automatically inherit the whole source session.
4. Receive the analysis, proposed graph changes and evidence references. Inspect the original request and applicable instructions before deciding what generalizes. Supported changes may update a node, description, relationship or schema only in the selected area. A skills-type node may hold procedure guidance; edits to shared native plugin skills/hooks affect all agents and need a separately scoped user request. A supported no-change result is valid.
