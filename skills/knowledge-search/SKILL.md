...
1. Read `.moneta.md`; invoke `evolve-init` if absent. Apply [memory selection](references/memory-scope.md) to the optional `agent-slug`; omission always selects general. Apply [reviewed state](references/reviewed-state.md) before reading any graph file. Then read only the selected area's indexes; verify an applicable enrolled responsibility when present. General without a role is valid. Completion: one selected memory area and its clean reviewed revision/path are known.
