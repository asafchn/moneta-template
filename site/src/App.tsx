  ...
  return <>
    ...
    <main id="main">
      ...
      <section id="install" className="section install-section"><div className="wrap">
        <SectionTitle number="04" eyebrow="A HOME FOR THE KNOWLEDGE" title="Set it up once. Keep making it yours." text="Moneta Setup creates your repository, then installs and enables your generated native plugin with its skills and hooks." />
        <div className="install-layout"><div className="terminal"><div className="terminal-top"><span>INSTALL MONETA SETUP</span><span>↙ your terminal</span></div><Tabs value={host} onChange={(_, value: string) => setHost(value)} aria-label="Agent host"><Tab label="Codex" value="codex" /><Tab label="Claude Code" value="claude-code" /></Tabs><div className="route-switch" role="group" aria-label="Installation method"><Button size="small" variant={installRoute === 'native' ? 'contained' : 'text'} onClick={() => setInstallRoute('native')} aria-pressed={installRoute === 'native'}>Native plugin</Button><Button size="small" variant={installRoute === 'skills' ? 'contained' : 'text'} onClick={() => setInstallRoute('skills')} aria-pressed={installRoute === 'skills'}>Via skills.sh</Button></div><pre data-testid="install-command">{commands}</pre><Button className="copy-button" variant="outlined" onClick={async () => { try { await navigator.clipboard.writeText(commands); setNotice('Install command copied'); } catch { setNotice('Clipboard unavailable. Select and copy the command above.'); } }}>Copy command <span className="arrow">⧉</span></Button><div className="invoke-note">Then open a fresh chat and run <code>{invocation}</code></div></div>
          <div className="wizard-description"><p className="eyebrow">THE WIZARD TAKES IT FROM HERE</p><ol><li><strong>Choose where it lives.</strong><span>GitHub or GitLab. New or existing repository. Your account or organization; private if you prefer.</span></li><li><strong>Tell it what the memory is for.</strong><span>Optional context shapes your README. Connect exact repositories or a GitLab group once.</span></li><li><strong>Review your first graph.</strong><span>The wizard installs your plugin. Merge the graph setup PR/MR and open a fresh chat; approve hook trust if your host asks.</span></li></ol></div>
        </div>
        <div className="setup-notes"><p><strong>Before you start</strong> Git, Node.js, your agent host, and authenticated <code>gh</code> or <code>glab</code>. This template is currently private; installation requires repository access.</p><p><strong>You control the boundary</strong> Organization matching reads only the current codebase’s local Git origin. No organization inventory or code scan. With write access blocked, setup prepares local files and explains the remaining manual steps.</p></div>
      </div></section>
      <section className="local-preview wrap"><div><p className="eyebrow">YOUR GRAPH, ON YOUR MACHINE</p><h2>See what your agent knows.</h2><p>Run <code>moneta-show</code> in your generated plugin. A local canvas opens your selected memory area, with node details and named connections. Custom types come from your own registry.</p></div><div className="local-command"><span>LOCALHOST ? READ ONLY</span><code>moneta-show</code><p>General by default. One explicit agent slug at a time. Your knowledge stays on your machine.</p></div></section><section className="closing wrap"><p className="eyebrow">LESS REPEATING. MORE BUILDING.</p><h2>The next session<br />starts with <em>what you learned.</em></h2><Button variant="contained" href="#install">Start with Moneta <span className="arrow">↗</span></Button></section>
    </main>
